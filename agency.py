...
topic = input("Enter Your Topic To Search Trends: ")
...
try :
    ...
    #the task 
    trend_task = Task(description=f""" Research the topic {topic} and identify the top 3 YouTube video angles that could go viral. Use internet search if needed. """, 
    ...
    tools= [search_tools],llm = llm,verbose = True)
    #task
    content_task = Task(description=""" Write a complete YouTube script based on the video ideas generated in the previous task. Choose the 1 best idea and expand it into a script. """,
    ...
    crew = Crew(agents= [trend_analyst,content_architect],tasks=[trend_task,content_task],verbose = True)
